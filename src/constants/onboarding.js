const QUESTIONNAIRE_ONBOARDING = [
  {
    title: "¡Bienvenido al cuestionario de Economía Circular!",
    text: "Para <strong>autoevaluar a tu empresa</strong> debes completar el siguiente cuestionario que tiene preguntas enfocadas en <strong>las 12 actividades de la Economía Circular.</strong>",
    backBtn: "Omitir",
    nextBtn: "Continuar",
    imgMobile: "onboarding-img1-mobile.png",
    imgDesktop: "onboarding-img1-desktop.png",
  },
  {
    title: "Las 12 actividades de la Economía Circular",
    text: "Serán <strong>explicadas</strong> a medida que avances en el desarrollo del <strong>cuestionario.<s/trong>",
    backBtn: "Regresar",
    nextBtn: "Continuar",
    imgMobile: "onboarding-img2-mobile.png",
    imgDesktop: "onboarding-img2-desktop.png",
  },
  {
    title: "Dentro del cuestionario",
    text: "Encontrarás preguntas de <strong>selección múltiple</strong> y de <strong>única opción</strong>, algunas requieren <strong>justificación</strong> para explicar más ampliamente lo que hace tu empresa.",
    backBtn: "Regresar",
    nextBtn: "Continuar",
    imgMobile: "onboarding-img3-mobile.png",
    imgDesktop: "onboarding-img3-desktop.png",
  },
  {
    title: "¡Podrás guardar todas tus respuestas!",
    text: "En caso de no <strong>terminar</strong> de responder todas las preguntas del cuestionario, puedes <strong>acceder</strong> de nuevo <strong>sin perder tu información.</strong>",
    backBtn: "Regresar",
    nextBtn: "¡Empezar el cuestionario!",
    imgMobile: "onboarding-img4-mobile.png",
    imgDesktop: "onboarding-img4-desktop.png",
  },
];

const REPORT_BY_LEVELS = [
  {
    title: "¡Bienvenido al reporte por niveles!",
    text: "En este reporte evaluamos las actividades que realiza tu empresa en <strong>tres diferentes niveles.</strong>",
    backBtn: "",
    nextBtn: "Continuar",
    imgMobile: "levels.svg",
    imgDesktop: "levels.svg",
  },
  {
    title: "En cada uno de estos niveles",
    text: "Se <strong>evalúan diferentes principios de la Economía Circular.</strong> Por ejemplo, en el nivel <strong>Cadena de Suministro</strong> evaluamos <strong>5</strong> principios tal como se puede apreciar en la imagen de la derecha.",
    backBtn: "Regresar",
    nextBtn: "Continuar",
    imgMobile: "pie-chart.png",
    imgDesktop: "pie-chart.png",
  },
  {
    title: "Cada principio evaluado en los diferentes niveles ",
    text: "Puede <strong>obtener hasta 1.000 puntos.</strong> Entonces, para el nivel <strong>Cadena de Suministro</strong>, dado que estamos evaluando <strong>5 principios</strong>, el <strong>puntaje máximo</strong> viene dado por: <br><br>5x1000 = <strong>5000 puntos</strong> ",
    backBtn: "Regresar",
    nextBtn: "Continuar",
    imgMobile: "progress-bar-report.svg",
    imgDesktop: "progress-bar-report.png",
  },
  {
    title: "En la tabla de puntajes de cada nivel",
    text: "Podrás <strong>comparar el puntaje posible</strong> con el <strong>puntaje obtenido</strong> de los principios evaluados, junto con su <strong>porcentaje de desempeño.</strong> ",
    backBtn: "Regresar",
    nextBtn: "Finalizar",
    imgMobile: "table-by-levels.svg",
    imgDesktop: "table-by-levels.png",
  },
];

const REPORT_ONBOARDING = [
  {
    title: "¡Bienvenido al reporte de resultados!",
    text: "Aquí puedes consultar los <strong>resultados de tu empresa</strong> y conocer su <strong>estado actual</strong> en relación con los principios de la economía circular.",
    backBtn: "",
    nextBtn: "Continuar",
    imgMobile: "report-questionnaire-mobile-1.png",
    imgDesktop: "report-questionnaire-1.png",
  },
  {
    title: "Inicialmente encontrarás el reporte por niveles",
    text: "Que te mostrará los puntajes obtenidos en tres áreas:<strong><br><br>1.Nivel cadena de suministro<br>2.Nivel empresa <br>3.Nivel producto</strong>",
    backBtn: "Regresar",
    nextBtn: "Continuar",
    imgMobile: "report-questionnaire-mobile-2.png",
    imgDesktop: "report-questionnaire-2.png",
  },
  {
    title: "Podrás acceder a una explicación detallada",
    text: "Para <strong>interpretar tus resultados de manera más precisa</strong> y comprender mejor tu diagnóstico del reporte por Niveles. <strong>Esta opción la encuentras en el botón “Ver explicación” ubicado en la esquina inferior derecha.</strong>",
    backBtn: "Regresar",
    nextBtn: "Continuar",
    imgMobile: "report-questionnaire-mobile-3.png",
    imgDesktop: "report-questionnaire-3.png",
  },
  {
    title: "Después, encontrarás el reporte de principios vs actividades",
    text: "Donde podrás <strong>visualizar el puntaje</strong> obtenido de las <strong>actividades</strong> que aportan en <strong>cada principio</strong> de la economía circular",
    backBtn: "Regresar",
    nextBtn: "Continuar",
    imgMobile: "report-questionnaire-mobile-4.png",
    imgDesktop: "report-questionnaire-4.png",
  },
  {
    title: "Del mismo modo, podrás acceder a una explicación detallada de este reporte",
    text: "Haciendo click en el <strong>botón “ver explicación”</strong> ubicado en la esquino inferior derecha, tal como se muestra en la imagen.",
    backBtn: "Regresar",
    nextBtn: "Continuar",
    imgMobile: "report-questionnaire-mobile-5.png",
    imgDesktop: "report-questionnaire-5.png",
  },
  {
    title: "Luego, encontrarás la hoja de ruta",
    text: "La cual te proporcionará una <strong>guía detallada</strong> que identificará las <strong>etapas clave</strong>, los hitos importantes y los recursos necesarios para lograr una <strong>evolución exitosa</strong> hacia la economía circular.",
    backBtn: "Regresar",
    nextBtn: "Continuar",
    imgMobile: "report-questionnaire-mobile-6.png",
    imgDesktop: "report-questionnaire-6.png",
  },
  {
    title: "Finalmente, te dejaremos algunas recomendaciones",
    text: "En base a los <strong>aspectos de menor desempeño</strong> identificados en la evaluación, para ayudar a tu empresa a <strong>avanzar hacia un modelo económico más circular.</strong>",
    backBtn: "Regresar",
    nextBtn: "Finalizar",
    imgMobile: "report-questionnaire-mobile-7.png",
    imgDesktop: "report-questionnaire-7.png",
  }
]

const PRINCIPLES_VS_ACTIVITIES = [
  {
    title: "¡Bienvenido al reporte por Principios vs Actividades!",
    text: "Existe una <strong>relación entre principios y actividades</strong> que permite la evaluación de ambos criterios, por tanto, las <strong>actividades</strong> pueden <strong>aportar puntos</strong> a varios <strong>principios.</strong>",
    backBtn: "",
    nextBtn: "Continuar",
    imgMobile: "report-principles-activities-1.png",
    imgDesktop: "report-principles-activities-1.png",
  },
  {
    title: "Para explicarte esta gráfica",
    text: "Tomaremos como ejemplo el <strong>Principio 1 (P1)</strong> de la Economía Circular: <strong>Capital Natural.</strong>",
    backBtn: "Regresar",
    nextBtn: "Continuar",
    imgMobile: "report-principles-activities-2.png",
    imgDesktop: "report-principles-activities-2.png",
  },
  {
    title: "P1: Capital Natural",
    text: "Como se puede observar en la gráfica, las <strong>actividades</strong> que <strong>aportan</strong> a la evalución de este principio son:<br><br><strong>A2:</strong>Mantener el equilibrio ecosistémico en las zonas de operación de la organización.<br><strong>A3:</strong>Devolver los recursos biológicos recuperados a la biósfera.",
    backBtn: "Regresar",
    nextBtn: "Continuar",
    imgMobile: "report-principles-activities-2.png",
    imgDesktop: "report-principles-activities-2.png",
  },
  {
    title: "El puntaje total de cada principio evaluado",
    text: "Es la <strong>suma del puntaje</strong> de las <strong>actividades</strong> que <strong>componen cada principio.</strong> Entonces, para el <strong>principio 1 (P1),</strong> el puntaje total es: <br><br><strong>Total puntos P1</strong> = 450 <strong>(A2)</strong> + 356 <strong>(A3)</strong> = <strong>806 puntos</strong>",
    backBtn: "Regresar",
    nextBtn: "Continuar",
    imgMobile: "report-principles-activities-3.png",
    imgDesktop: "report-principles-activities-3.png",
  },
  {
    title: "Debes tener en cuenta que",
    text: "<strong>Las actividades pueden aportar puntos a varios principios de la economía circular.</strong> Por ejemplo, como se observa en la imagen, la <strong>actividad 2 (A2)</strong> aporta a los principios: <strong>P1, P3 y P9.</strong>",
    backBtn: "Regresar",
    nextBtn: "Continuar",
    imgMobile: "report-principles-activities-4.png",
    imgDesktop: "report-principles-activities-4.png",
  },
  {
    title: "¡Recuerda!",
    text: "La calificación de <strong>12.000 puntos</strong> representa el <strong>estado ideal</strong> de la empresa en términos de <strong>Economía Circular</strong>, pues cada principio puede obtener hasta <strong>1.000 puntos.</strong><br><br><strong>Las actividades</strong> presentan <strong>un valor diferente</strong> al de los principios. Puedes consultar los valores en la <strong>Tabla de Puntajes</strong> que se muestra en el reporte.",
    backBtn: "Finalizar",
    nextBtn: "Continuar",
    imgMobile: "report-principles-activities-5.png",
    imgDesktop: "report-principles-activities-5.png",
  },
]

const PRINCIPLES = [
  {
    title: "",
    text: "<span class='custom-text-color-purple font-bold'>1.Capital Natural</span><br><br><p class='text-xs'>Fortalecer y mantener el desarrollo de una economía que no comprometa las funcionalidades ecosistémicas, controlando las reservas finitas y equilibrando los flujos de recursos renovables, devolviendo los nutrientes a los ecosistemas.</p><br><span class='custom-text-color-purple font-bold'>2. Tecnologías para la Ec</span><br><br><p class='text-xs'>Utilizar y/o desarrollar tecnologías que promueven la EC (fuentes de energía renovables en sus operaciones, tecnologías para el agua, para el suelo, para las emisiones).</p>",
    backBtn: "",
    nextBtn: "Continuar",
    imgMobile: "principles.png",
    imgDesktop: "principles.png",
  },
  {
    title: "",
    text: "<span class='custom-text-color-purple font-bold'>3. Pensamiento sistémico</span><br><br><p class='text-xs'>Enfoque holístico que permite comprender las complejidades, dinámicas e interconexiones entre los procesos y actores de la cadena de valor con respecto a flujos de recursos, provisión de servicios, gobernanza, marcos regulatorios, mercado, infraestructura e innovación y prácticas del usuario que pueden soportar la transición hacia la Economía Circular.</p><br><span class='custom-text-color-purple font-bold'>4. Pensamiento en cascada</span><br><br><p class='text-xs'>Principio basado en la posibilidad de incrementar el valor de una materia prima o secundaria para introducirla en otra parte del ciclo de vida del mismo producto o en otros eslabones de la cadena de suministro.</p>",
    backBtn: "Regresar",
    nextBtn: "Continuar",
    imgMobile: "principles.png",
    imgDesktop: "principles.png",
  },
  {
    title: "",
    text: "<span class='custom-text-color-purple font-bold'>5. Ecoeficiencia</span><br><br><p class='text-xs'>El rendimiento tiene que ser sinérgico y basado en la creación de beneficios múltiples, incluyendo la creación de valores agregados, de puestos de trabajo y la reducción del consumo de recursos.</p><br><span class='custom-text-color-purple font-bold'>6. Innovación</span><br><br><p class='text-xs'>Las organizaciones continuamente innovan para crear valor empresarial a través de la sostenibilidad, gestión de recursos en productos y servicios.</p>",
    backBtn: "Regresar",
    nextBtn: "Continuar",
    imgMobile: "principles.png",
    imgDesktop: "principles.png",
  },
  {
    title: "",
    text: "<span class='custom-text-color-purple font-bold'>7. Gestión del impacto</span><br><br><p class='text-xs'>Las organizaciones gestionan el impacto directo e indirecto de sus decisiones y actividades, por medio de prácticas sostenibles con sus grupos de interés de manera clara, precisa, oportuna, honesta y completa.</p><br><span class='custom-text-color-purple font-bold'>8. Optimización de valor</span><br><br><p class='text-xs'>Las organizaciones mantienen todos los productos, componentes y materiales en su valor y utilidad más altos en todo momento.</p>",
    backBtn: "Regresar",
    nextBtn: "Continuar",
    imgMobile: "principles.png",
    imgDesktop: "principles.png",
  },
  {
    title: "",
    text: "<span class='custom-text-color-purple font-bold'>9. Pensamiento local</span><br><br><p class='text-xs'>Las organizaciones y comunidades están influenciadas con su contexto y por eso tiene una relación dinámica y simbiótica de proximidad. En los ecosistemas ocurre de forma similar, es importante que los habitantes o clientes se sientan familiarizados con los productos o servicios que van a adquirir y participen de las actividades de recuperación de valor.</p><br><span class='custom-text-color-purple font-bold'>10. Resiliencia para la diversidad</span><br><br><p class='text-xs'>Implica que los productos y servicios se adapten a distintos contextos a lo largo de su ciclo de vida, siendo más eficientes, simples, modulares, robustos y al mismo tiempo versátiles.</p>",
    backBtn: "Regresar",
    nextBtn: "Continuar",
    imgMobile: "principles.png",
    imgDesktop: "principles.png",
  },
  {
    title: "",
    text: "<span class='custom-text-color-purple font-bold'>11. Circularidad y longevidad</span><br><br><p class='text-xs'>Que el producto tenga la capacidad de regenerarse y a su vez perdure en el tiempo conservando su máxima utilidad.</p><br><span class='custom-text-color-purple font-bold'>12. Diseño para prevención de residuos</span><br><br><p class='text-xs'>Los productos y servicios se pueden concebir y diseñar de manera que se minimice la generación de residuos para integrarlos a los ciclos biológicos y técnicos.</p>",
    backBtn: "Regresar",
    nextBtn: "Finalizar",
    imgMobile: "principles.png",
    imgDesktop: "principles.png",
  },

]


export default {
  QUESTIONNAIRE_ONBOARDING,
  REPORT_BY_LEVELS,
  REPORT_ONBOARDING,
  PRINCIPLES_VS_ACTIVITIES,
  PRINCIPLES
};

export const QUESTIONNAIRE_ONBOARDING_KEY = 'questionnaireOnboardingCompleted';
export const REPORT_ONBOARDING_KEY = 'reportOnboardingCompleted';